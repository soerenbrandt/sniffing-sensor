    ...
    for dataset in sets:
        ...
        for chem, set_numbers in dataset.items():
            for num in set_numbers:
                derivs.append(exp_derivs[num] /
                              np.sqrt(np.sum(exp_derivs[num]**2)))
                labels.append(chem)
        ...
        for n in range(0, len(values)):
            if by == 'Concentration':
                label_df = pd.read_sql_query(
                    ...
                    + str(values_df["Analyte ID"][n]), conn)
                ...
                label_df = pd.read_sql_query(
                    'SELECT "Analyte ID", "' + by +
                    '" FROM AnalyteData WHERE "Analyte ID" IS ' +
                    str(values_df["Analyte ID"][0]), conn)
...
def data_dimensionality(data):
    ...
    print("Dimensions at 1%: " +
          str(np.argmax(pca.explained_variance_ratio_ < 0.01))
          )  # report total variance captured by PCA
    print("Captured variance: " + str(
        (pca.explained_variance_ratio_[:10].round(2)))
          )  # report total variance captured by PCA
