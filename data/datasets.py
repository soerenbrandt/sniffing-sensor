...
COLOR_DICT = OrderedDict(
    [
        ("Pentane", (69.0 / 255, 127.0 / 255, 181.0 / 255)),  # blue
        ("Hexane", (238.0 / 255, 124.0 / 255, 48.0 / 255)),  # orange
        ("Heptane", (96.0 / 255, 183.0 / 255, 96.0 / 255)),
        ("Octane", (192.0 / 255, 46.0 / 255, 38.0 / 255)),  # red
        ("Nonane", (174.0 / 255, 140.0 / 255, 205.0 / 255)),
        ("Decane", (168.0 / 255, 128.0 / 255, 119.0 / 255)),
        ("Ethanol", (233.0 / 255, 152.0 / 255, 209.0 / 255)),
        ("Water", (158.0 / 255, 158.0 / 255, 158.0 / 255)),
        ("Acetone", (204.0 / 255, 205.0 / 255, 89.0 / 255)),
        ("Toluene", (80.0 / 255, 206.0 / 255, 218.0 / 255)),
        ("Acetonitrile", (100.0 / 255, 180.0 / 255, 180.0 / 255)),
    ]
)
    ...
    def __init__(self):
        ...
        exp_set = OrderedDict(
            [
                ...
                (
                    "Hexane",
                    [175, 184, 185, 186, 187, 199, 204, 205, 424, 425],
                ),  # 149 new cuvette (measurement error?), 282 short cuvette?
                # ('Cyclohexane', [155, 180]),
                (
                    "Heptane",
                    [150, 206, 207],
                ),  # 176 new cuvette (measurement error?)
                ("Octane", [151, 177, 208, 209, 242, 243, 244, 283]),
                ("Nonane", [152, 178, 188, 189, 267]),
                (
                    "Decane",
                    [153, 179, 190, 191, 197, 268, 284],
                ),  # 198 new decane (measurement error?)
                # ('Dodecane', [192]),
                # ('Methanol', [156, 172, 193, 196]),
                (
                    "Ethanol",
                    [157, 171, 324, 330, 440, 441, 442],
                ),  # 325 measurement error?
                # ('Isopropanol', [158, 173, 200, 201]),
                (
                    "Water",
                    [169, 170, 312, 313, 443, 444, 445],
                ),  # 330 short cuvette?, 342 sucrose?
...
class OriginalSVCSet(Dataset):
    def __init__(self):
        """Experimental set used for PCA experimentation by Tim and/or Sean"""
        exp_set = OrderedDict(
            [
                ("Pentane", [239, 240, 241, 434, 435]),  # 203,248
                (
                    "Hexane",
                    [204, 592, 593, 594, 571, 570],
                ),  # 130,185,138,139,184,185,141,199
...
def _all_compounds(existing_CSV=True, **kwargs):
    ...
    for analyteID in analytes_df["Analyte ID"]:
        ...
        while (
            n <= 4
            and analytes_df.loc[analytes_df["Analyte ID"] == analyteID][
                f"Concentration {n}"
            ].values[0]
            is not None
            and float(
                analytes_df.loc[analytes_df["Analyte ID"] == analyteID][
                    f"Concentration {n}"
                ].values[0]
            )
            > 0
    ...
    exp_df = pd.read_sql_query(
        ...
        + ", ".join(map(str, _exclude_from_dataset()))
        + ")",
        conn,
    )
    ...
    if existing_CSV == True:
        ...
        if (
            bool(set(list(exp_df.Experiment_ID)).difference(existing_IDs))
            == True
        ):
            print(
...
def _binary_mixtures(chem_list=None, **kwargs):
    ...
    pure_df = pd.read_sql_query(
        ...
        + chem_list[0]
        + '" AND "Component 2" IS NULL) OR ("Component 1" LIKE "'
        + chem_list[1]
        + '" AND "Component 2" IS NULL)',
        conn2,
    )
    mixtures_df = pd.read_sql_query(
        'SELECT "Analyte ID", "Component 1", "Concentration 1", "Component 2", "Concentration 2" FROM AnalyteData WHERE ("Component 1" LIKE "'
        + chem_list[0]
        + '" AND "Component 2" LIKE "'
        + chem_list[1]
        + '") OR ("Component 1" LIKE "'
        + chem_list[1]
        + '" AND "Component 2" LIKE "'
    ...
    mixtures_sorted_df = mixtures_df.sort_values(
        by=["Concentration 1"], inplace=False
    )
    ...
    def __init__(self):
        exp_set = OrderedDict(
            [
                ...
                ),  # np.arange(37,46) pure compounds
                ("incomplete", []),
            ]
        )
        super().__init__(exp_set)
        ...
        exp_set = OrderedDict(
            [
                (
                    ...
                    + [618, 619, 620]
                    + [623],
                ),  # 601, 604, 621, 622 # 629-2.0s injection
                (
                    "C5C8 1",
                    [239, 275]
                    + range(241, 246)
        ...
        datasets = {
            "glass": OrderedDict(
                [
                    (
                        0.0,
                        range(312, 314) + [169, 170] + [691] + range(443, 446),
                    ),
...
class TernaryGradients(Dataset):  # add binary + unary references
    def __init__(self):
        ...
        """
        exp_set = OrderedDict(
            [("C5C6C8", range(231, 239)), ("EtMeDIW", range(426, 434))]
        )
        super().__init__(exp_set)
    ...
    def __init__(self):
        exp_set = OrderedDict(
            [
                ...
                (
                    "Sivs13F 2",
                    [170, 406, 324] + [915, 776, 760] + [879, 891, 907],
                ),
            ]
        )
        super().__init__(exp_set)
